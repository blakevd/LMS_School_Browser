    ...
    public class RegisterModel : PageModel
    {
        ...
        string CreateNewUser( string firstName, string lastName, DateTime DOB, string departmentAbbrev, string role )
        {
            if (role == "Administrator")
            {
                ...
                var aQuery = from v in db.Administrators select v.UId;
                var sQuery = from v in db.Students select v.UId;
                var pQuery = from v in db.Professors select v.UId;
                var uidTable = aQuery.Union(sQuery).Union(pQuery).ToList();
                string maxuid = (aQuery.Union(sQuery).Union(pQuery).ToList().Max());

                string result = "u0000000";
                if (maxuid != null)
                {
                    int u = Int32.Parse(maxuid.Substring(1)) + 1;

                    result = result.Remove(result.Length - u.ToString().Length);
                    result += u.ToString();
                }
