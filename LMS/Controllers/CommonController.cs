    ...
    public class CommonController : Controller
    {
        ...
        public IActionResult GetClassOfferings(string subject, int number)
        {
            var query =
                ...
                where course.Subject == subject && course.Number == number

                select new { season = cl.Semester, year = cl.Year, location = cl.Location, start = cl.Start, end = cl.End, fname = p.FName, lname = p.LName };
        ...
        public IActionResult GetUser(string uid)
        {
            ...
            from p in db.Professors
            where p.UId == uid
            select new { fname = p.FName, lname = p.LName, uid = p.UId, department = p.Department };

            if (query1.Count() > 0)
                return Json(query1.Single());
